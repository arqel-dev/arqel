...
**Service Provider + Config.** `WorkflowServiceProvider` é auto-discovered via `extra.laravel.providers`, boota via `Spatie\LaravelPackageTools\PackageServiceProvider` (`name('arqel-workflow')`, `hasConfigFile('arqel-workflow')` + `hasMigrations(...)`), e registra o listener canônico de história. `config/arqel-workflow.php` expõe blocos `default_color`, `default_icon`, `audit`, `broadcast_transitions`, `authorization`, `history` (a chave `user_model` **não** é publicada — `StateTransition::user()` a lê via `config('arqel-workflow.user_model', 'App\\Models\\User')` com default no código).
