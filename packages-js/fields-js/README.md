...
Rich React input components for [Arqel](https://arqel.dev), registered into `@arqel/ui`'s FieldRegistry.
...
```bash
pnpm add @arqel/fields @arqel/ui @arqel/react @arqel/types
```
...
import '@arqel/fields/register'; // side effect: registers all built-ins
...
import { NumberInput, CurrencyInput } from '@arqel/fields/number';
import { Checkbox, Toggle } from '@arqel/fields/boolean';
```
