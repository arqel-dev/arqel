...
> - **Rodada 8 (2026-07-09/10):** **Milestone 0.20 (API-freeze prep) CONCLUÍDO + v0.18.0 PUBLICADO.** v0.18.0 saiu (tag cortada, release 25/25, npm+Packagist) entregando o milestone 0.19 completo (Plugin API #362, Global Search #364, Database Notifications #366) + fix de segurança do SecurityScanner #367. O 0.20 fechou com: doc-sweeps `05-api-php`/`06-api-react` alinhados ao código (#370/#372), superfície pública marcada `@internal` + `SharedProps.tenant` tipado (#373), `07-roadmap-fases.md` sincronizado com o ADR-019 (#374), e product docs completos — reference/php + guides + resources (#375). **Todos os 4 must-haves de 1.0 entregues e publicados; API-freeze prep completo.** O ADR-019 (aceite) rege o critério de 1.0. **Próximo gate rumo a 1.0: milestone 0.9x — Hardening** (cobertura fields-js/ui/react ao target; E2E auth-avançado/export/marketplace; piloto de produção).
> - **Rodada 7 (2026-07-07):** **Milestone 0.19 quase completo.** Plugin API no Panel ENTREGUE (`Panel::plugin()` + contrato `Contracts\Plugin`, #362, sessão paralela) → lacuna competitiva #5 fechada. Global Search de registros ENTREGUE (`RecordSearchCommandProvider` + `Resource::globallySearchable()`, #364, aditivo-PHP: React/endpoint intocados) → lacuna #6 fechada. Database Notifications UI (lacuna #4) em finalização por sessão paralela (`feat/db-notifications`). Com isso, **os 4 must-haves de 1.0 (Imports + Relation Managers + Plugin API + Notifications/Search) ficam todos entregues ou em finalização**. Próximo foco: 0.20 (API-freeze prep — ADR-019 já aceite; resta fechar tipos TS e docs `resources/*`).
...
| **Notifications (database)** | ✅ HAVE | Flash: `useFlash.ts:12`; Database Notifications UI entregue (#366, v0.18.0): sino read/unread no topbar + histórico paginado (`/admin/notifications`), sobre `DatabaseNotification` nativas do Laravel; `NotificationController` escopado ao usuário (anti-IDOR) | Flash + notificações persistidas. Broadcast realtime deferido a 0.19b. |
| **Global search (registros cross-resource)** | ✅ HAVE | `RecordSearchCommandProvider.php` (milestone 0.19); `Resource::globallySearchable()` opt-in; `globalSearchResultTitle()` override | Busca full-text across-resource via Command Palette (Cmd+K). Resource declara `globallySearchable()` com nomes de coluna; `RecordSearchCommandProvider` executa LIKE escopada, `viewAny`-gated, link para edit. |
...
| **Plugin API registrável no Panel** | ✅ HAVE | `Panel::plugin(Plugin)` / `Panel::plugins(array)` (`Panel.php:251/264`); contrato `Contracts\Plugin` (`register(Panel)` / `boot(Panel)`); marketplace para distribuição (`marketplace/src/Models/Plugin.php`) | Extensibilidade in-code entregue (milestone 0.19, #362): um plugin implementa `Plugin::register()`/`boot()` e injeta resources/pages/widgets via `->plugin()`. Distribuição via marketplace complementa. |
...
| **0.19 — Extensibilidade + Notifications** ✅ **CONCLUÍDO** | ✅ Plugin API no Panel (`->plugin()`, #362) + Database Notifications UI (#366) + Global Search de registros (#364) — publicado em v0.18.0 | Lacunas #4/#5/#6 |
| **0.20 — API-freeze prep** ✅ **CONCLUÍDO** | ✅ ADR-019 aceite; divergências B–H resolvidas + doc `resources/*` (#375); doc `05/06-api` alinhada (#370/#372); `@internal` + `SharedProps.tenant` tipado (#373); roadmap-fases sincronizado (#374) | Pré-requisito duro de 1.0 |
...
4. ✅ **ADR-019** (freeze + SemVer) + reescrita do critério de versionamento — **aceite e concluído** no milestone 0.20 (2026-07): ADR escrito (`03-adrs.md`), superfície pública vs `@internal` declarada (#373), doc alinhada ao código (#370/#372/#375), critério de versionamento reescrito (`07-roadmap-fases.md` #374).
