...
## CANDIDATE #8 — destructive built-in Delete (row) loses its required ConfirmDialog when demoted to the dropdown ActionMenu → one-click irreversible delete with NO confirmation

**Surfaced:** Phase-5 E2E reconciliation (spec 01 Post-resource row/bulk delete), then adversarially re-verified against a FRESH JS dist rebuild.
**Severity (suspected):** HIGH — a destructive, irreversible action executes on a single click with no confirmation gate, despite the framework explicitly declaring that gate. This is a data-loss safety defect, arguably more severe than #7A (which fails *safe* — the custom action 404s and does NOT execute; this one *executes*).

**The gap:** the built-in row Delete action is declared with a mandatory confirmation server-side — `Actions::delete()` (`packages/actions/src/Actions.php:40`) calls `->requiresConfirmation()` with heading "Delete record?" / "This action cannot be undone." / destructive color, and `Action::toArray()` (`packages/actions/src/Action.php:339`) serializes `requiresConfirmation: true` into the React payload. The INLINE action path honors it: `ActionMenu` (`packages-js/ui/src/action/ActionMenu.tsx:50-64`) renders inline actions (<=3 on a row) via `<ActionButton>`, which mounts `ConfirmDialog` (`ActionButton.tsx:74-117`). But the DROPDOWN path (when a row has >3 actions, exceeding the inline threshold) calls `onInvoke(action)` directly inside `DropdownMenuItem.onSelect` (`ActionMenu.tsx:82-89`), **bypassing `ConfirmDialog`/`ActionFormModal` entirely** — the same root as #7A, but here the built-in Delete HAS a real stock URL (`Action.php:402` -> `DELETE /admin/{slug}/{id}`), so instead of a harmless 404 it **fires the real destructive request immediately**.

**Reproduction (showcase, fresh dist):** PostResource has 4 row actions (edit, delete, publish, change_status) > the inline threshold of 3, so all collapse into the "Actions" dropdown. Opening the dropdown and clicking "Delete" issues `DELETE /admin/posts/{id}` -> 303 (the row is gone, list re-fetches) with **zero** ConfirmDialog/alertdialog rendered in the DOM. Confirmed against the freshly-rebuilt dist (2026-06-08), so it is NOT a stale-dist artifact — it reproduces on current source HEAD.

**Regression trigger:** before the showcase added the `publish` + `change_status` custom actions (Task 3.1), the Post row had only 2 actions (edit, delete) -> INLINE -> delete's ConfirmDialog appeared (the original spec 01 clicked a real confirm button). Adding the 2 custom actions pushed the row to 4 -> dropdown -> delete silently lost its confirmation. So **any Resource that grows past 3 row actions loses delete confirmation** — a common, easily-hit configuration.

**Distinct from / related to:** same ROOT as #7A (dropdown `onSelect -> onInvoke` bypasses confirm/form modals), but a DISTINCT and more severe CONSEQUENCE: #7A is a custom action with no stock URL -> POSTs to a missing endpoint -> 404 -> fails safe (no mutation). #8 is a built-in action with a real stock URL + `requiresConfirmation()` -> executes the irreversible delete with no gate. The fix for #7A (route the dropdown `onSelect` through the same `ActionButton`/`ConfirmDialog` flow as inline) ALSO fixes #8 — so they likely share one PR in Round 22, but #8 should be called out explicitly as the safety-critical facet.

**Bulk-delete note (stale-dist correction):** an earlier live-stack observation that bulk "Delete selected" also deleted directly without a confirm dialog was a STALE-DIST ARTIFACT — the stack had been serving a 2026-06-05 `@arqel-dev/ui` dist predating commit `10cbc2a` ("gate bulk actions behind the confirm dialog"). Against the freshly-rebuilt dist, bulk "Delete selected" DOES render the ConfirmDialog and fires no delete request until confirmed. So the BULK path is already fixed on HEAD; only the per-row DROPDOWN path (#7A/#8) remains broken. (This is also why the showcase now rebuilds the JS packages before serving — see the stack/CI build step — so the dogfood E2E always exercises current source, not a stale dist.)

**Spec impact:** `01-resources.spec.ts`'s row-delete + bulk-delete tests were reconciled to the current UI. The row-delete test should HONESTLY encode #8 (assert the dropdown Delete fires `DELETE /admin/posts/{id}` with NO intervening ConfirmDialog) so that when Round 22 fixes the bypass, the test FAILS and forces re-introducing the confirm-dialog assertion — mirroring how `05-actions.spec.ts` honestly encodes #7A.
