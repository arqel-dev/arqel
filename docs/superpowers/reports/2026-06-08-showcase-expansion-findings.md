...
- `@arqel-dev/theme` — a SEPARATE `ThemeProvider`/`useTheme`/`ThemeToggle`/`LocaleSwitcher`-adjacent surface (`packages-js/theme/src/`), with its OWN `ThemeContext` (`packages-js/theme/src/ThemeProvider.tsx:6`) and richer props (`storageKey`, `darkClass`, `attribute`, `preventFlashScript`). This is the package the showcase task wires in and that any app would `import { ThemeProvider, ThemeToggle } from '@arqel-dev/theme'`.
