...
final class ProcessImportJob implements ShouldQueue
{
    ...
    /**
     * Guard against a missing required-mapping header. Reads only the
     * first row's keys (the reader yields rows keyed by header) so this
     * stays a cheap, streaming-friendly check — an empty file (no rows
     * at all) cannot be validated this way and is left to per-row rules.
     *
