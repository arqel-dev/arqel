...
O pacote `@arqel-dev/a11y` expõe primitivos reutilizáveis. **Sempre prefira-os** a reescrever
...
- **vitest-axe** em testes de componente:
  ...
  ```
- **`pnpm --filter @arqel-dev/a11y audit:scan`** — sweep heurístico de `.tsx` em `apps/`
...
`aria-controls`, `aria-activedescendant`. Pacote `@arqel-dev/ui` já encapsula isso — prefira
