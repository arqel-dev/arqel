...
- **Command Palette built-in providers (CMDPAL-002):**
  ...
  - `Arqel\Core\CommandPalette\Providers\RecordSearchCommandProvider` — construtor toma `ResourceRegistry`; `provide()` itera `$registry->all()`, carrega records por busca full-text (LIKE escopado, min 2 chars, até 10 por resource) nas colunas declaradas por `Resource::globallySearchable(): array` (opt-in), emite uma `Command` por record com id `record:{model_class}:{key}`, label `{globalSearchResultTitle($record)}` (method override quando presente, fallback `'#' . $record->getKey()`), url para a página de edição do record, category `'Records'`. **Autorização (#129):** cada record é filtrado via a Policy `viewAny` da Resource — resultados só incluem records que o `$user` pode ver. SQL é sempre bound (nenhum SQL injection via colunas de busca).
  - **Coverage:** 35 testes unit (`tests/Unit/CommandPalette/` — Registry, FuzzyMatcher, Command value-object, Navigation/Theme/RecordSearch providers) + 2 feature (`tests/Feature/CommandPalette*Test.php`). Total core test count: **154** (suite completa, todos verdes).
