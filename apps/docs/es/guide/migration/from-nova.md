...
> Guía de migración con 12 mappings side-by-side.
...
5. **El queueing de Action es igual, y el modal de formulario de la action ya viene built-in.** Arqel soporta el "modal de confirmación con fields" de Nova de forma nativa: declara los inputs con `Action::form([...fields])` (y `modalSize()`), y el `ActionFormModal` de `@arqel-dev/ui` renderiza el form de fields dentro del modal de la action — sin necesidad de un form React personalizado.
