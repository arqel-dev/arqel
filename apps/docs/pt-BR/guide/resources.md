...
O método `table()` é opcional — Arqel cai em derivação automática quando ausente. Actions de linha e em lote são declaradas dentro de `table()` via `->actions([...])`:
...
Veja [Tables & Forms](/pt-BR/guide/tables-forms) e [Actions](/pt-BR/guide/actions).
...
- [Fields](/pt-BR/guide/fields) — catálogo dos 21 inputs
- [Actions](/pt-BR/guide/actions) — botões e bulk operations
- [Auth](/pt-BR/guide/auth) — Policies + field-level auth
